...
class _BaseStore:
    ...
    def ingest(self, facts: list[MemoryFact]) -> int:
        ...
        if self._embedder is None:  # auto-embedding: o Atlas gera na indexacao
            vectors = [None] * len(clean)
        else:
        ...
        for fact in clean:
            self._audit.emit(
                ...
                embedder=self._embedder.name if self._embedder else "atlas-auto",
            )
        return n

    # -- leitura --------------------------------------------------------
    def search(self, query: str, k: int = 3, scope: str | None = None) -> list[dict]:
        qvec = self._embedder.embed_query(query) if self._embedder else None
        results = self._vector_search(qvec, k=k, scope=scope, query_text=query)
    ...
    def _vector_search(self, qvec, k: int, scope: str | None, query_text: str = "") -> list[dict]:
        raise NotImplementedError
...
class LocalMemoryStore(_BaseStore):
    ...
    def _vector_search(self, qvec, k, scope, query_text=""):
        scored = []
...
class AtlasMemoryStore(_BaseStore):
    ...
    def _persist(self, facts, vectors) -> int:
        ...
        for fact, vec in zip(facts, vectors):
            ...
            if vec is not None:  # no auto-embedding o Atlas gera e guarda o vetor
                doc["embedding"] = vec
            docs.append(doc)
    ...
    def _vector_search(self, qvec, k, scope, query_text=""):
        stage = build_vector_search_stage(
            self._settings, query_vector=qvec, query_text=query_text, k=k, scope=scope
        )
        pipeline = [
            stage,
            {"$project": {
...
def build_vector_search_stage(settings, *, query_vector, query_text, k, scope=None):
    """Monta o estagio $vectorSearch nos dois modos.

    - Embedding no cliente (voyage/openai/local): path "embedding" + queryVector.
    - Auto-embedding (provedor "atlas"): path "text" + query em texto puro;
      o Atlas gera o embedding da consulta com o modelo do indice.
      Sintaxe do modo auto conforme preview do recurso; se o Atlas recusar o
      estagio, confira a doc de Automated Embedding para a forma vigente."""
    vs: dict = {
    ...
    if query_vector is not None:
        ...
        vs["path"] = "text"
        vs["query"] = {"text": query_text}
        vs["model"] = settings.auto_embed_model
    if scope:
        vs["filter"] = {"scope": scope}
...
def build_memory_store(settings: Settings | None = None) -> MemoryStore:
    ...
    embedder = None if provider == "atlas" else create_embedding_provider(provider, settings)
